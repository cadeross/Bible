...
import * as SheetPrimitive from "@radix-ui/react-dialog" // Using Dialog as base for Sheet
...
>(({ className, ...props }, ref) => (
    <SheetPrimitive.Overlay
        className={cn(
            "fixed inset-0 z-50 bg-black/30 backdrop-blur-[8px] transition-all data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=open]:duration-500 data-[state=closed]:duration-500",
            className
        )}
        {...props}
        ref={ref}
...
const sheetVariants = cva(
    "fixed z-50 gap-4 glass border-white/[0.12] dark:border-white/[0.06] p-6 shadow-[var(--shadow-dialog)]",
    {
        variants: {
            side: {
                top: "inset-x-0 top-0 rounded-b-2xl border-b",
                bottom: "inset-x-0 bottom-0 rounded-t-2xl border-t",
                left: "inset-y-0 left-0 h-full w-3/4 rounded-r-2xl border-r sm:max-w-sm",
                right: "inset-y-0 right-0 h-full w-3/4 rounded-l-2xl border-l sm:max-w-sm",
            },
        },
        defaultVariants: {
            side: "right",
